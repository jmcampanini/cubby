...
func LoadProject(startDir string) (*Project, error) {
	...
	hostCfg, err := loadRequiredFile[HostConfig](hostFile)
	if err != nil {
...
func loadRegisteredSource(hostRoot string, index int, source HostSource) (RegisteredSource, error) {
	...
	sourceCfg, err := loadRequiredFile[SourceConfig](sourceFile)
	if err != nil {
...
func loadRequiredFile[T any](path string) (T, error) {
	var zero T
	loader, err := configloader.NewRequiredFileLoader[T](path)
	if err != nil {
		return zero, err
	}
	cfg, _, err := configloader.Load(zero, loader)
	if err != nil {
		return zero, err
	}
	return cfg, nil
